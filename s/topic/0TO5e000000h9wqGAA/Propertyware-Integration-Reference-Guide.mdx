...
2. **Work Orders (WO) -** The integration will not update any fields on previously created or existing work orders.

3. **Leases -** We update all values if it is an existing lease. So whatever new values come in from the sync would override the old ones.
